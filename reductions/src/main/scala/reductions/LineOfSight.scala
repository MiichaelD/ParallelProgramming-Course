...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    output(0) = 0f // location of the observer
    var index = 1
    while (index < input.length) {
      output(index) = Math.max(input(index) / index, output(index - 1)) // height / distance from viewing point
      index += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    while (index < until) {
      maxAngle = Math.max(input(index) / index, maxAngle)
      index += 1
    }
    maxAngle
  }
   ...
   */
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int,
                          until: Int): Unit = {
    if (until == from) {
      return
    }
    ...
    while (index < until) {
      maxAngle = Math.max(maxAngle, input(index) / index)
